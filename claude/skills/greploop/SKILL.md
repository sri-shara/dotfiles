...
MAX_GREPTILE_ATTEMPTS=60
GREPTILE_ATTEMPT=0

while true; do
  GREPTILE_ATTEMPT=$((GREPTILE_ATTEMPT + 1))
  if [ "$GREPTILE_ATTEMPT" -gt "$MAX_GREPTILE_ATTEMPTS" ]; then
    echo "Timed out waiting for Greptile check. Verify Greptile is configured for this repository."
    exit 1
  fi
...
Then poll for the Greptile pipeline job to complete:
...
while true; do
  GREPTILE_ATTEMPT=$((GREPTILE_ATTEMPT + 1))
  if [ "$GREPTILE_ATTEMPT" -gt "$MAX_GREPTILE_ATTEMPTS" ]; then
    echo "Timed out waiting for Greptile pipeline. Verify Greptile is configured for this repository."
    exit 1
  fi
...
**GitHub** — fetch unresolved review threads and resolve all that have been addressed:
...
**GitLab** — fetch unresolved discussions and resolve each one:
